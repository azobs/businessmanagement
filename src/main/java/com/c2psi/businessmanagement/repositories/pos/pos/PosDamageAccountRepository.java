...
public interface PosDamageAccountRepository
    ...
    @Query("SELECT posdamacc FROM PosDamageAccount posdamacc WHERE posdamacc.pdaArticle.id=:artId AND posdamacc.pdaPointofsaleId=:posId")
    Optional<PosDamageAccount> findPosDamageAccountofArticleInPos(@Param("artId") Long artId, @Param("posId") Long posId);
    @Query("SELECT posdamacc FROM PosDamageAccount posdamacc WHERE posdamacc.pdaPointofsaleId=:posId ORDER BY posdamacc.pdaArticle.artPf.pfProduct.prodCat.catName ASC, posdamacc.pdaArticle.artName")
